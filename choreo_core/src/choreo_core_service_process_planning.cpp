...
bool ChoreoCoreService::generateMotionLibrary(
    const int selected_path_index, choreo_core_service::TrajectoryLibrary& traj_lib)
{
...
ProcessPlanResult ChoreoCoreService::generateProcessPlan(const int& selected_path_index)
{
  ProcessPlanResult result;

  bool fetch_co_success = false;
  std::vector<choreo_msgs::UnitProcessPlan> process_plan;
  ...
  choreo_msgs::TaskSequencePlanning ts_srv;
  ...
  choreo_msgs::ProcessPlanning srv;
  ...
  if(choreo::ASSEMBLY_TYPE_SPATIAL_EXTRUSION == assembly_type_)
  {
    ...
    else
    {
      ...
      ts_srv.request.element_array = std::vector<choreo_msgs::ElementCandidatePoses>(
...
{
  choreo_msgs::MoveToTargetPose srv;
