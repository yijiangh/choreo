...
{
  if (!choreo_param_helpers::toFile(filename, graph_list_msg))
  {
                             ...
                             bool use_saved_graph)
{
  ...
  choreo_process_planning::DescartesTraj sol;
  ...
  if (use_saved_graph)
  {
    if (choreo_param_helpers::fromFile(saved_graph_file_name, graph_list_msg))
    {
  ...
  trajectory_msgs::JointTrajectory ros_traj = choreo_process_planning::toROSTrajectory(sol, *model);
  ...
  for (size_t i = 0; i < task_seq.size(); i++)
  {
    ...
    sub_process.process_type = choreo_msgs::SubProcess::PROCESS;
    sub_process.main_data_type = choreo_msgs::SubProcess::CART;

    switch (task_seq[i].type)
    {
      case choreo_msgs::ElementCandidatePoses::SUPPORT:
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::SUPPORT;
        break;
      }
      case choreo_msgs::ElementCandidatePoses::CREATE:
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::CREATE;
        break;
      }
      case choreo_msgs::ElementCandidatePoses::CONNECT:
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::CONNECT;
        break;
      }
      default:
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::NONE;
                             ...
                             bool use_saved_graph)
{
  ...
  for (size_t i = 0; i < segs.size(); i++)
  {
    ...
    sub_process.unit_process_id = i;
    sub_process.process_type = choreo_msgs::SubProcess::RETRACTION;
    sub_process.main_data_type = choreo_msgs::SubProcess::CART;
    ...
    for(int k = 0; k < 4; k++)
    {
      if(k % 2 == 0)
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::APPROACH;
      }
      else
      {
        sub_process.element_process_type = choreo_msgs::SubProcess::DEPART;
      }
