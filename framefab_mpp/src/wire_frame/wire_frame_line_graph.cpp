...
framefab::wire_frame::WireFrameLineGraph::WireFrameLineGraph()
...
void framefab::wire_frame::WireFrameLineGraph::LoadFromOBJ(const char *path)
{
  ...
  try
  {
    ...
    while (fgets(pLine, 512, fp))
    {
      ...
      {
        std::vector <WF_vert*> bound_points;
        tok = strtok(pLine, " ");
        char tmp[128];
...
void framefab::wire_frame::WireFrameLineGraph::LoadFromPWF(const char *path)
{
...
void framefab::wire_frame::WireFrameLineGraph::WriteToOBJ(const char *path)
{
  using std::vector;
  using std::fill;
...
void framefab::wire_frame::WireFrameLineGraph::WriteToPWF(
...
void framefab::wire_frame::WireFrameLineGraph::ImportFrom3DD(const char *path)
{
  FILE *fp = fopen(path, "r");
...
void framefab::wire_frame::WireFrameLineGraph::ExportSubgraph(const char *path)
{
  FILE *fp = fopen(path, "wb+");
  int N = SizeOfVertList();
  int M = SizeOfEdgeList();
...
void framefab::wire_frame::WireFrameLineGraph::ExportPoints(int min_layer, int max_layer, const char *path)
{
  ...
  std::vector<bool> is_export_vert(N);
  std::fill(is_export_vert.begin(), is_export_vert.end(), false);
...
void framefab::wire_frame::WireFrameLineGraph::ExportLines(int min_layer, int max_layer, const char *path)
{
  if ((*pedge_list_)[0]->Layer() == -1)
  {
    min_layer = -(1 << 20);
...
framefab::wire_frame::WF_vert* framefab::wire_frame::WireFrameLineGraph::InsertVertex(Vec3f p)
{
  // detect duplication
  int N = SizeOfVertList();
  for (int i = 0; i < N; i++)
...
framefab::wire_frame::WF_edge* framefab::wire_frame::WireFrameLineGraph::InsertEdge(WF_vert *u, WF_vert *v)
{
  // detect duplication
  WF_edge *e = u->pedge_;
  while (e != NULL)
...
framefab::wire_frame::WF_edge* framefab::wire_frame::WireFrameLineGraph::InsertOneWayEdge(WF_vert *u, WF_vert *v)
{
  if (u == v)
  {
    return NULL;
...
void framefab::wire_frame::WireFrameLineGraph::Unify()
{
  maxx_ = -1e20;
  maxy_ = -1e20;
  maxz_ = -1e20;
...
trimesh::point framefab::wire_frame::WireFrameLineGraph::Unify(Vec3f p)
{
  return (p - center_pos_) * scaleV_;
}

void framefab::wire_frame::WireFrameLineGraph::SimplifyFrame()
{
  int N = SizeOfVertList();
  int M = SizeOfEdgeList();

  std::vector<bool> delete_vert;
  delete_vert.resize(N);
  std::fill(delete_vert.begin(), delete_vert.end(), false);

  std::vector<bool> delete_edge;
  delete_edge.resize(M);
  std::fill(delete_edge.begin(), delete_edge.end(), false);
  ...
  std::vector<WF_vert *>::iterator itv = pvert_list_->begin();
  ...
  std::vector<WF_edge *>::iterator ite = pedge_list_->begin();
...
void framefab::wire_frame::WireFrameLineGraph::ProjectBound(double len)
{
  if (base_vert_ == 0)
  {
    return;
...
void framefab::wire_frame::WireFrameLineGraph::ModifyProjection(double len)
{
...
void framefab::wire_frame::WireFrameLineGraph::MakeCeiling(std::vector<WF_edge*> &bound_e)
{
  if (bound_e.size() == 0)
  {
    return;
...
void framefab::wire_frame::WireFrameLineGraph::MakeBase(std::vector<WF_vert*> &base_v)
{
  if (base_v.size() == 0)
  {
    return;
...
void framefab::wire_frame::WireFrameLineGraph::MakeSubGraph(std::vector<WF_edge*> &subg_e)
{
  if (subg_e.size() == 0)
  {
    return;
