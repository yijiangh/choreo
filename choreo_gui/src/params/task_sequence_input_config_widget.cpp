...
choreo_gui::TaskSequenceInputConfigWidget::TaskSequenceInputConfigWidget(choreo_msgs::TaskSequenceInputParameters params)
    : params_(params)
{
...
void choreo_gui::TaskSequenceInputConfigWidget::update_display_fields()
{
  ui_->lineedit_filepath->setText(QString(params_.file_path.c_str()));
}

void choreo_gui::TaskSequenceInputConfigWidget::update_internal_fields()
{
  params_.file_path = ui_->lineedit_filepath->text().toLocal8Bit().constData();
  last_filepath_ = params_.file_path;
}

void choreo_gui::TaskSequenceInputConfigWidget::get_file_path_handler()
{
