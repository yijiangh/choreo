...
void convertParsedAssemblyTypeString(const std::string& p_at, choreo_gui::SelectionWidget::ASSEMBLY_TYPE& at)
{
  if("spatial_extrusion" == p_at)
  {
    at = choreo_gui::SelectionWidget::ASSEMBLY_TYPE::SPATIAL_EXTRUSION;
    return;
  }

  if("picknplace" == p_at)
  {
    at = choreo_gui::SelectionWidget::ASSEMBLY_TYPE::PICKNPLACE;
    return;
  }
...
choreo_gui::SelectionWidget::SelectionWidget(QWidget* parent) : QWidget(parent),
...
//  connect(task_seq_recompute_pop_up_, SIGNAL(exitSelectForPlanPopUpWidget()), this, SLOT(popUpWindowClosed()));
  ...
  visualize_client_ =
      nh_.serviceClient<choreo_msgs::VisualizeSelectedPath>(VISUALIZE_SELECTED_PATH);

  // call core node to fetch element number in (1) parsed assembly seq or (2) computed plans
  query_computation_record_client_ =
      nh_.serviceClient<choreo_msgs::QueryComputationRecord>(QUERY_COMPUTATION_RESULT);
}

void choreo_gui::SelectionWidget::loadParameters()
{
  choreo_msgs::ElementNumberRequest srv;

  switch (mode_)
  {
    case PATH_SELECTION:
    {
      srv.request.action = choreo_msgs::ElementNumberRequest::Request::REQUEST_ELEMENT_NUMBER;
      break;
    }
    case PLAN_SELECTION:
    {
      srv.request.action = choreo_msgs::ElementNumberRequest::Request::REQUEST_SELECTED_TASK_NUMBER;
      break;
    }
    default:
    {
  ...
  ros::ServiceClient element_number_param_client =
      nh_.serviceClient<choreo_msgs::ElementNumberRequest>(ELEMENT_NUMBER_REQUEST_SERVICE);
...
void choreo_gui::SelectionWidget::setMaxGraspNum(int m)
{
  assert(m > 0);
  max_grasp_num_ = m - 1;
...
void choreo_gui::SelectionWidget::orderValueChanged()
{
  ...
  choreo_msgs::VisualizeSelectedPath srv;
...
void choreo_gui::SelectionWidget::setStatusBar(std::string string, bool state)
{
  if(state)
  {
...
void choreo_gui::SelectionWidget::buttonSelectForPlan()
{
  ...
  choreo_msgs::QueryComputationRecord srv;
  bool saved_record_found = false;

  srv.request.action = choreo_msgs::QueryComputationRecordRequest::SAVED_LADDER_GRAPH;
...
{
  Q_EMIT enableChoreoWidgetButtons();
}